...
public class EntitystateTest {
    ...
    void test1() {
        ...
        try {
            ...
            Memo memo = new Memo(); // 비영속 상태, 그냥 객체만 만든 상태임
                                    // JPA가 관리하고 있는 상태가 아니라 순수한 Entity Class상태인거임
            memo.setId(1L);
            memo.setUsername("Robbie");
            memo.setContents("비영속과 영속 상태");

            em.persist(memo);  // 관리하고 싶은 정보를 던지면 영속성에서 관리함
                                // persist를 통해서 비영속상태를 영속성 컨텍스트에 저장한다.
                                // Entity가 관리되고 있는 상태로 변경된 경우 영속상태라고 부른다
            et.commit();
